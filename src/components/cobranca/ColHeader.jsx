...
export default function ColHeader({ label, field, data, filters, setFilters, t, sortKey, sortCfg, onSort, width }) {
  ...
  const allChecked = !selected || selected.length === allValues.length;
  const someChecked = selected && selected.length > 0 && selected.length < allValues.length;

  function toggle(val) {
    const cur = selected || allValues;
    const next = cur.includes(val) ? cur.filter(x => x !== val) : [...cur, val];
    setFilters(f => ({ ...f, [field]: next.length === allValues.length ? null : next }));
  }
  function selectAll() { setFilters(f => ({ ...f, [field]: null })); }
  function clearAll() { setFilters(f => ({ ...f, [field]: [] })); }
  ...
  return (
    <th ref={ref} style={thStyle}>
      <div style={{ display: "flex", alignItems: "stretch" }}>
        ...
        <button onClick={() => setOpen(x => !x)} style={{ background: hasFilter ? t.p : "none", border: "none", cursor: "pointer", padding: "0 8px", fontSize: 11, color: hasFilter ? "#fff" : t.muted, borderLeft: `1px solid ${t.bor}33`, display: "flex", alignItems: "center" }} title="Filtrar">
          {hasFilter ? "▼" : "⌄"}
        </button>
      </div>

      {open && (
        <div style={{ position: "absolute", top: "100%", left: 0, zIndex: 500, background: t.drop, border: `1px solid ${t.p}`, borderRadius: 6, boxShadow: "0 6px 24px rgba(0,0,0,.25)", minWidth: 220, maxWidth: 300 }}>
          ...
          <div style={{ display: "flex", gap: 6, padding: "6px 10px", borderBottom: `1px solid ${t.bor}` }}>
            <button onClick={selectAll} style={{ flex: 1, background: t.th, border: `1px solid ${t.bor}`, borderRadius: 4, padding: "4px 0", fontSize: 11, cursor: "pointer", color: t.txt, fontWeight: 600 }}>Selec. Tudo</button>
            <button onClick={clearAll} style={{ flex: 1, background: t.th, border: `1px solid ${t.bor}`, borderRadius: 4, padding: "4px 0", fontSize: 11, cursor: "pointer", color: "#ef4444", fontWeight: 600 }}>Limpar</button>
          </div>
          <div style={{ maxHeight: 220, overflowY: "auto", padding: "4px 0" }}>
            ...
            {filtered.map(val => {
              const chk = !selected || selected.includes(val);
              return (
          ...
          <div style={{ padding: "8px 10px", borderTop: `1px solid ${t.bor}`, display: "flex", justifyContent: "flex-end", gap: 8 }}>
            <button onClick={() => setOpen(false)} style={{ background: t.p, border: "none", borderRadius: 5, padding: "5px 18px", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>OK</button>
            <button onClick={() => { setFilters(f => ({ ...f, [field]: null })); setOpen(false); }} style={{ background: t.th, border: `1px solid ${t.bor}`, borderRadius: 5, padding: "5px 12px", color: t.txt, fontWeight: 600, fontSize: 12, cursor: "pointer" }}>Cancelar</button>
          </div>
        </div>
      )}
    </th>
