...
export function keyPart(v) {
  return String(v ?? "").toUpperCase().replace(INVISIBLE_DEDUPE, "").replace(/\s+/g, "").replace(/\./g, "").replace(/^0+(\d+)$/, "$1");
}
...
function displayTituloNumero(titulo) {
  return String(titulo ?? "").replace(INVISIBLE_DEDUPE, "");
}
...
function clientKeyForTitle(item = {}) {
  const cod = String(item.nrCli ?? item.client_code ?? item.clientCode ?? item["Código Cliente"] ?? "").replace(/\D/g, "").replace(/^0+(\d+)$/, "$1");
  if (cod) return `C:${cod}`;
  const nome = normalizarRazaoSocial(item.nomeCli ?? item.client_name ?? item["Nome Cliente"] ?? "");
  return nome ? `N:${nome}` : "";
}

export function getTituloKey(item = {}) {
  const origem = item.origem ?? item.source;
  const titulo = item.titulo ?? item.title_number ?? item["Número Documento"];
  const seq = item.seq ?? item["Sequência"];
  const vencimento = item.vencimento ?? item.due_date ?? item["Data Vencimento"];
  ...
  const cli = clientKeyForTitle(item);
  return [normOrig, cli, numeroNorm, seqNorm, vencNorm].join("|");
}
...
export function buildId({ origem, nrCli, tp, titulo, seq }) {
  return [origem, nrCli, tp, displayTituloNumero(titulo), seq].map(keyPart).join("|");
}
...
export function buildItem(o) {
  ...
  const valorEmAberto = num(o.valorEmAberto ?? o.open_value ?? (saldoErp > 0 ? saldoErp : Math.max(0, valorOriginal - valorRecebido)));
  ...
  return {
    ...o,
    id: buildId(o),
    titulo: displayTituloNumero(o.titulo),
    valorOriginal,
    valorRecebido,
    valorEmAberto,
    saldoErp,
...
export function dbToItem(r) {
  return buildItem({
    _dbId: r.id,
    origem: origemExibicaoCarteira(r.source),
    nrCli: r.client_code,
    ...
    tp: r.doc_type,
    ser: r.serie,
    titulo: r.title_number_display || r.title_number,
    seq: r.seq,
    ...
    valorEmAberto: r.open_value ?? r.original_value,
    saldoErp: r.erp_balance,
    partialPaymentDetected: r.partial_payment_detected,
    portador: r.portador,
    status: r.current_status || "Não Contatado",
    encaminhar: normalizeWorkflowStatusForCarteira(r.workflow_status || ""),
    workflow_status: normalizeWorkflowStatusForCarteira(r.workflow_status || ""),
    workflow_status_diagnostico: r.workflow_status === "sem_carteira" ? "sem_carteira" : "",
...
function parseTotalCliente(row) {
  ...
  return {
    telefone: String(raw.match(/(?:Tel\.?|Telefone)\s*:?\s*(.+?)(?:\s+Contato\s*:|$)/i)?.[1] || "").trim(),
    contato: String(raw.match(/Contato\s*:?\s*(.+)$/i)?.[1] || "").trim(),
  };
}
...
function buildFinr1253ItemFromTitulo(row, clienteAtivo, dadosTotal = {}) {
  ...
  const valorAberto = receber > 0 ? receber : Math.max(0, valorFaceTit - recebPrc);
  if (valorAberto <= 0 && valorFaceTit <= 0) return null;
  ...
  return buildItem({
    ...
    valorOriginal: valorFaceTit || valorAberto,
    valorRecebido: recebPrc,
    valorEmAberto: valorAberto || valorFaceTit,
    valorTotalDebito: valorAberto || valorFaceTit,
    valorMulta: 0, valorJuros: calculada, acrescimo, recebPrc, valorCalculado: calculada, valorReceber: receber,
    atrasoRelatorio: atraso, uteis, portador, partialPaymentDetected: recebPrc > 0,
  });
}
...
export function parseRows1253(rows) {
  ...
  for (const row of rows) {
    ...
    if (!joined) continue;
    const isHeaderRow = firstNorm === "TP" || firstNorm === "TPSER" || firstNorm === "TIPO" || (joined.includes("VENCTO") && joined.includes("ATRASO") && !firstNorm.startsWith("CLIENTE"));
    if (headerSkipped < 2 || isHeaderRow) { if (!isHeaderRow) headerSkipped += 1; continue; }
  ...
  console.info(`FINR1253: ${dedup.length} lançamentos importados. VAL. ORIG = Vlr Título; Total a Cobrar = Receber/Saldo.`);
  return dedup;
}
...
function detectHeaderRow7007(rows) {
  for (let i = 0; i < Math.min(rows.length, 20); i += 1) {
    ...
    for (const k of Object.keys(row)) {
      const n = normH(k);
      for (const aliases of Object.values(H7007)) if (aliases.some((a) => n === a || n.includes(a) || a.includes(n))) { matches += 1; break; }
    }
...
export function parseRows7007(rows) {
  ...
  for (const row of rows.slice(headerIdx + 1 > 0 ? headerIdx + 1 : 1)) {
    ...
    const nrCli = hmap.nrCli && row[hmap.nrCli] != null ? String(row[hmap.nrCli]).replace(/\D/g, "").trim() : "";
    ...
    if (!isValidClientName(nome) && !nrCli) { descartados += 1; addMotivo("sem_cliente"); continue; }
    if (!titulo) { descartados += 1; addMotivo("sem_titulo"); continue; }
    if (!vencimento && valorTotalErp <= 0 && valorEmAberto <= 0) { descartados += 1; addMotivo("sem_venc_nem_valor"); continue; }
    if (valorEmAberto <= 0) { descartados += 1; addMotivo("saldo_zerado"); continue; }
    ...
    out.push(buildItem({
      ...
      valorOriginal: valorTotalErp || valorEmAberto,
      valorRecebido: valorRecebidoErp,
      valorEmAberto,
      valorTotalDebito: valorEmAberto,
      saldoErp,
  ...
  console.info(`RPT_7007: ${dedup.length} títulos importados | ${descartados} descartados | motivos:`, motivosDescarte, "VAL. ORIG = Vlr Título; Total a Cobrar = Saldo.");
  return dedup;
}
