...
export default function Dashboard() {
  ...
  const groupedFiltrado = useMemo(() => {
    ...
    return grouped
      .filter((g) => {
        ...
        if (!showPaid) {
          const temPagamento = g.statusConsolidado === "Encerrado" || g.statusConsolidado === "Baixado" || g.statusConsolidado === "Pago Aguard. Baixa" || g.statusConsolidado === "Confirmado" || g.historicoCliente.some((h) => h.motivo === "Confirmado");
  ...
  const dash = useMemo(() => {
    ...
    return {
      ...
      aCobrar: base.reduce((s, x) => s + x.valorOriginal, 0),
      vMultaTotal: base.reduce((s, x) => s + x.valorMulta, 0),
      vJurosTotal: base.reduce((s, x) => s + x.valorJuros, 0),
      vTotComJuros: base.reduce((s, x) => s + x.valorOriginal + x.valorMulta + x.valorJuros, 0),
  ...
  async function limparDuplicatasBanco() {
    ...
    try {
      ...
      setCleanupMsg(`🔍 ${totalFisico} registros encontrados. Agrupando...`);
      await yieldUI();
      ...
      if (totalDupRegistros === 0) {
        setCleanupMsg(`✅ Limpeza finalizada: nenhuma duplicata encontrada. ⏱${((performance.now() - tTotal) / 1000).toFixed(1)}s`);
        return;
      }

      const ok = window.confirm(`📊 ${totalFisico} registros | ${gruposDup.length} grupos com duplicata | ${totalDupRegistros} a inativar.\n\nDeseja prosseguir?`);
      ...
      for (const group of gruposDup) {
        ...
        for (const dup of group.slice(1)) {
          ...
          toInativar.push({ id: dup.id, payload: { active: false, current_motive: `Duplicata inativada ${hojeISO}`, workflow_status: "duplicata", updated_by: `Limpeza ${hojeISO}` }});
        }
      }

      if (toInativar.length === 0) {
        setCleanupMsg(`✅ Todas duplicatas preservadas por dados manuais. ⏱${((performance.now() - tTotal) / 1000).toFixed(1)}s`);
        return;
      }
      ...
      for (let i = 0; i < toInativar.length; i += LOTE) {
        setCleanupMsg(`⏳ Inativando — lote ${Math.floor(i / LOTE) + 1}/${totalLotes}...`);
        await Promise.all(toInativar.slice(i, i + LOTE).map(({ id, payload }) => base44.entities.Titulo.update(id, payload)));
        await yieldUI();
      }

      setCleanupMsg(`✅ Limpeza concluída — ${toInativar.length} inativados | ${preservadosManuais} preservadas | ⏱${((performance.now() - tTotal) / 1000).toFixed(1)}s`);
      await loadData();
    } catch (err) {
      setCleanupMsg(`❌ Erro: ${err.message}`);
    }
  }
  ...
  async function syncImport(source, imported, fileName, onProgress) {
    ...
    for (const item of deduped) {
      ...
      const financeiro = {
        source: item.origem, client_code: item.nrCli || null, client_name: item.nomeCli,
        doc_type: item.tp || null, serie: item.ser || null, title_number: item.titulo,
        seq: item.seq || null, nf_servico: item.nfServico || null, issue_date: item.emissao || null,
        due_date: item.vencimento || null, original_value: Number(item.valorOriginal || 0),
        portador: item.portador || null, active: true, import_file: fileName,
      };
      if (!old) {
        toCreate.push({ ...financeiro, current_status: "Não Contatado", current_motive: null, current_contact_type: null, client_category: null, promise_date: null, last_contact_date: null, last_note: null, contact_count: 0, protest_requested_by: null, workflow_status: "normal", updated_by: "Importação" });
      } else {
        const mudou = (
    ...
    for (let i = 0; i < toUpdate.length; i += CONCUR) {
      onProgress(`🔄 Atualizando — lote ${Math.floor(i / CONCUR) + 1}/${Math.ceil(toUpdate.length / CONCUR)}...`);
  ...
  async function importarArquivo(e) {
    ...
    try {
      ...
      if (isCobrDia(cleanRows.length ? cleanRows : rawRows)) {
        ...
        const srcLabel = source === "FINR1253" ? "FINR1253 (Topcon)" : "RPT_7007 (EB)";
        setStep(`Processando ${srcLabel}...`);
        await yieldUI();
    ...
    <div style={{ fontFamily: "'Segoe UI',system-ui,sans-serif", background: t.bg, minHeight: "100vh", color: t.txt }}>
      ...
      <header style={{ background: t.head, borderBottom: `1px solid ${t.bor}`, padding: "0 20px", height: 50, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: t.shad }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <Btn t={t} sm onClick={limparDuplicatasBanco} style={{ background: "#64748b", border: "none", color: "#fff" }}>🧹 Limpar BD</Btn>
      ...
      <main style={{ padding: "14px 16px", maxWidth: "100%", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto", paddingBottom: 8, paddingTop: 8, scrollbarWidth: "thin", WebkitOverflowScrolling: "touch", alignItems: "stretch", justifyContent: "flex-start", borderBottom: `1px solid ${t.bor}` }}>
        ...
        {activeTab === "carteira" && <><div className="kpi-container kpi-container-8"><KPI t={t} label="Total em Aberto" color="#F59E0B" value={fmtM(dash.vTotComJuros)} sub={`Orig: ${fmtM(dash.aCobrar)} · Multa: ${fmtM(dash.vMultaTotal)} · Juros: ${fmtM(dash.vJurosTotal)}`} /><KPI t={t} label="A Cobrar" color="#EF4444" value={fmtM(dash.aCobrar)} sub={`Multa: ${fmtM(dash.vMultaTotal)} · Juros: ${fmtM(dash.vJurosTotal)}`} onClick={() => setKpiFilter((p) => p === "aCobrar" ? null : "aCobrar")} active={kpiFilter === "aCobrar"} /><KPI t={t} label="Cobrado" color="#10B981" value={fmtM(dash.cobrado)} sub="já contactados" onClick={() => setKpiFilter((p) => p === "cobrado" ? null : "cobrado")} active={kpiFilter === "cobrado"} /><KPI t={t} label="Cobrados Hoje" color="#FBBF24" value={dash.cobHoje} sub={`${dash.perc.toFixed(1).replace(".", ",")}% do total`} onClick={() => setKpiFilter((p) => p === "cobHoje" ? null : "cobHoje")} active={kpiFilter === "cobHoje"} /><KPI t={t} label="Faltam Cobrar" color="#EF4444" value={dash.faltando} sub="sem contato hoje" onClick={() => setKpiFilter((p) => p === "faltando" ? null : "faltando")} active={kpiFilter === "faltando"} /><KPI t={t} label="Nº Clientes" color="#6B7280" value={dash.numCli} sub="ativos" /><KPI t={t} label="Nº Títulos" color="#6B7280" value={dash.numTit} sub="ativos" /><KPI t={t} label="Val. Original" color="#10B981" value={fmtM(dash.vOrig)} sub="sem multa/juros" /></div>{kpiFilter && <div style={{ textAlign: "center", marginBottom: 12 }}><button onClick={() => setKpiFilter(null)} style={{ background: t.p, border: "none", borderRadius: 6, padding: "6px 14px", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 11 }}>✕ Limpar Filtro</button></div>}</>}
