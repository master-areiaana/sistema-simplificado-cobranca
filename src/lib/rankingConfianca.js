...
const STATUS_PAGO = ["PAGO AGUARD BAIXA", "PAGO AGUARD. BAIXA", "ENCERRADO", "BAIXADO", "CONFIRMADO", "PAGAMENTO CONFIRMADO", "PAGO"];
const STATUS_PROMESSA = ["PROMETEU PAGAR", "PROMESSA", "PROMESSA ATIVA", "PROMESSA DE PAGAMENTO"];
...
function eventDate(e = {}) {
  return e.event_date || e.dataContato || e.data || e.created_date || "";
}

function eventPromise(e = {}) {
  return e.promise_date || e.dataPromessa || e.promessa || "";
}
...
function eventosDoCliente(grupo = {}, events = []) {
  ...
  return (events || [])
    .filter(e => {
      const hasIdentity = !!(e.client_code || e.nrCli || e.client_name || e.nomeCli);
...
export function statusAutomaticoCobranca(frm = {}) {
  ...
  if (frm.encaminhar === "verificacao") return "Pago aguardando baixa";
  if (frm.dataPromessa) return frm.dataPromessa < hojeISO ? "Promessa vencida / cobrança necessária" : "Promessa ativa";
  return "Em Cobrança";
}
...
export function rankingConfiancaCliente(grupo = {}, events = []) {
  const baseEvents = events?.length ? events : (grupo.historicoCliente || []);
  const hist = eventosDoCliente(grupo, baseEvents);
  const promessas = hist.filter(e => isPromessa(e.status, eventPromise(e)));
  const pagos = hist.filter(e => isPago(e.status));
  const pagamentosPorData = new Set(pagos.map(e => dateOnly(eventDate(e))).filter(Boolean));
  ...
  for (const p of promessas) {
    const promessa = dateOnly(eventPromise(p));
  ...
  if (pagamentoNaPromessa && promessasQuebradas === 0) return { nivel: 1, label: "Ranking 1 · Cliente confiável", cor: "#10b981", acao: "Pagou conforme promessa. Pode receber nova chance controlada." };
  if (pagamentoNaPromessa && promessasQuebradas === 1) return { nivel: 2, label: "Ranking 2 · Segunda chance", cor: "#f59e0b", acao: "Pagou na segunda promessa. Manter acompanhamento." };
  if (pagamentoNaPromessa && promessasQuebradas === 2) return { nivel: 3, label: "Ranking 3 · Baixa confiabilidade", cor: "#f97316", acao: "Pagou somente na terceira promessa. Acompanhar com prioridade." };
  if (promessasQuebradas >= 3) return { nivel: 4, label: "Cliente não confiável", cor: "#ef4444", acao: "Passou da terceira promessa. Criar ação formal e sinalizar gestão/assessoria." };
  if (promessaAtualVencida || promessasQuebradas > 0) return { nivel: 0, label: "Promessa vencida", cor: "#ef4444", acao: "Promessa vencida sem pagamento. Recobrar e registrar nova ação." };
  if (grupo.dataPromessa) return { nivel: 0, label: "Promessa ativa", cor: "#3b82f6", acao: `Aguardar pagamento prometido para ${fmtD(grupo.dataPromessa)}.` };
