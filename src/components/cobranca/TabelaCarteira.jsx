...
export default function TabelaCarteira({ sortedCart, baseCart, fCart, setFCart, selected, toggleSel, toggleAll, scCart, handleSort, setModal, setForm, setHistModal, openCli, setOpenCli, emptyForm, isDark, t, makeColData, fieldVal, applyExcelFilter, setNegModal, onEncaminharSugestao }) {
  ...
  return (
    <div>
      ...
      <div style={{ borderRadius: 10, border: `1px solid ${t.bor}`, boxShadow: t.shad, maxHeight: "62vh", overflowY: "auto", overflowX: "hidden", width: "100%" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <thead>
            <tr>
              <th style={{ ...thS(t), textAlign: "center" }}>
              ...
              <CH label="CLIENTE" field="nomeCli" data={makeColData(baseCart, "nomeCli")} filters={fCart} setFilters={setFCart} sortKey="cliente" />
              ...
              <th style={thS(t)}>CLASSIF.</th>
              <th style={thS(t)}>SUGESTÃO</th>
              <CH label="OBSERVAÇÃO" field="obsConsolidada" data={makeColData(baseCart, "obsConsolidada")} filters={fCart} setFilters={setFCart} />
              <th style={thS(t)}>AÇÕES</th>
            </tr>
          </thead>
          <tbody>
            {sortedCart.length === 0 && (
              <tr><td colSpan={20} style={{ textAlign: "center", padding: 44, color: t.muted, background: t.surf }}>Nenhum resultado. Verifique os filtros.</td></tr>
            )}
            {sortedCart.map((g, i) => {
              ...
              return (
                <React.Fragment key={g.clientKey}>
                  <tr style={{ background: rowBg, borderLeft: `4px solid ${leftClr}` }}>
                    ...
                    <td style={tdS()}><button onClick={() => setOpenCli(p => ({ ...p, [g.clientKey]: !p[g.clientKey] }))} style={{ background: "transparent", border: `1px solid ${t.bor}`, color: t.txt, borderRadius: 4, cursor: "pointer", padding: "1px 5px", fontSize: 11 }}>{open ? "−" : "+"}</button></td>
                    <td style={{ ...tdS(), color: t.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.nrCli}</td>
                    <td style={{ ...tdS(), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={g.nomeCli}><b>{g.nomeCli}</b></td>
                    <td style={{ ...tdS(), textAlign: "center" }}>{g.qtdTitulos}</td>
                    <td style={{ ...tdS(), color: t.muted, fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fmtD(g.primeiroVencimento)}</td>
                    <td style={{ ...tdS(), color: g.maiorAtraso > 0 ? "#ef4444" : "#10b981", fontWeight: 700 }}>{g.maiorAtraso > 0 ? `${g.maiorAtraso}d` : "—"}</td>
                    <td style={{ ...tdS(), fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fmtM(g.valorOriginal)}</td>
                    <td style={{ ...tdS(), color: "#f97316", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fmtM(g.valorMulta)}</td>
                    <td style={{ ...tdS(), color: "#eab308", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fmtM(g.valorJuros)}</td>
                    <td style={{ ...tdS(), fontWeight: 800, color: t.p, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fmtM(g.valorTotalDebito)}</td>
                    <td style={{ ...tdS(), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 10 }}>{g.statusConsolidado}</td>
                    <td style={tdS()}>{encBadge(g.encaminharConsolidado)}</td>
                    <td style={tdS()}>
                      {[...new Set(g.titulos.map(x => x.origem))].map(o => (
                        <span key={o} style={{ display: "inline-block", fontSize: 8, background: o === "FINR1253" ? "#7c3aed22" : "#0369a122", color: o === "FINR1253" ? "#7c3aed" : "#0369a1", padding: "1px 4px", borderRadius: 3, fontWeight: 700 }}>{o === "FINR1253" ? "TC" : "EB"}</span>
                      ))}
                    </td>
                    <td style={{ ...tdS(), color: t.muted, fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fmtD(g.ultimoContato)}</td>
                        ...
                        <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
                          ...
                          {onEncaminharSugestao && (sugestao.label === "Protesto" || sugestao.label === "Verificar" || sugestao.label === "Assessoria" || sugestao.label === "Jurídico") && (
                            <button
                              ...
                              style={{ background: sugestao.cor, color: "#fff", border: "none", borderRadius: 3, padding: "1px 5px", fontSize: 9, fontWeight: 700, cursor: "pointer" }}
                            >→</button>
                          )}
                        </div>
                      ) : null}
                    </td>
                    <td style={{ ...tdS(), maxWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><ObsCell text={g.obsConsolidada} t={t} /></td>
                    <td style={tdS()}>
                  ...
                  {open && g.titulos.map(item => (
                    <tr key={item.id} style={{ background: t.surf2 }}>
                      ...
                      <td style={{ ...tdS(), color: t.muted, fontSize: 10 }}>{item.status}</td>
                      <td style={tdS()}>{encBadge(item.encaminhar)}</td>
                      <td colSpan={7} style={{ ...tdS(), color: t.muted, fontSize: 10 }}>{item.portador || "—"}</td>
                    </tr>
                  ))}
                </React.Fragment>
              );
