...
function localKey(entityName) { return `${LOCAL_PREFIX}${entityName}`; }
function readLocal(entityName) { if (!isBrowser) return []; try { return JSON.parse(window.localStorage.getItem(localKey(entityName)) || '[]'); } catch { return []; } }
function notify(entityName) { for (const callback of subscribers.get(entityName) || []) { try { callback(); } catch { /* noop */ } } }
function writeLocal(entityName, rows) { if (!isBrowser) return; window.localStorage.setItem(localKey(entityName), JSON.stringify(rows || [])); notify(entityName); }
function nowISO() { return new Date().toISOString(); }
function ensureId(record = {}) { return record.id || `local_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`; }
...
function saveLocal(entityName, record = {}) {
  ...
  const saved = { ...previous, ...record, id, created_date: previous.created_date || record.created_date || nowISO(), updated_date: nowISO() };
...
function makeLocalEntity(entityName) {
  return {
    async list(orderBy, limit = 1000) { return prepareTituloRows(entityName, sortRows(readLocal(entityName), orderBy).slice(0, limit)); },
    async filter(criteria = {}, orderBy, limit = 1000) { return prepareTituloRows(entityName, sortRows(readLocal(entityName).filter((row) => matches(row, criteria)), orderBy).slice(0, limit)); },
    async create(record) { return saveLocal(entityName, record); },
    async update(id, fields) { return saveLocal(entityName, { ...(fields || {}), id }); },
    async bulkCreate(records = []) { const result = []; for (const record of records || []) result.push(await this.create(record)); return result; },
    subscribe(callback) { const set = subscribers.get(entityName) || new Set(); set.add(callback); subscribers.set(entityName, set); return () => { const next = subscribers.get(entityName) || new Set(); next.delete(callback); subscribers.set(entityName, next); }; },
  };
}
...
function makeHybridEntity(entityName) {
  ...
  return {
    async list(orderBy, limit = 1000) {
      ...
      try { const rows = await runRemote(() => remote.list(orderBy, limit)); if (Array.isArray(rows) && rows.length > 0) { mergeLocal(entityName, rows); return prepareTituloRows(entityName, rows); } }
    ...
    async filter(criteria = {}, orderBy, limit = 1000) {
      ...
      try { const rows = await runRemote(() => remote.filter(criteria, orderBy, limit)); if (Array.isArray(rows) && rows.length > 0) { mergeLocal(entityName, rows); return prepareTituloRows(entityName, rows); } }
      catch (error) { console.warn(`[local-first] filter ${entityName} local`, error); }
      return fallback;
    },
    async create(record) { const localSaved = await local.create(record); const remote = remoteEntity(entityName); if (!remote?.create) return localSaved; try { const saved = await runRemote(() => remote.create(record)); return saved?.id ? saveLocal(entityName, saved) : localSaved; } catch (error) { console.warn(`[local-first] create ${entityName} local`, error); return localSaved; } },
    async update(id, fields) { const localSaved = await local.update(id, fields); const remote = remoteEntity(entityName); if (!remote?.update) return localSaved; try { const saved = await runRemote(() => remote.update(id, fields)); return saved?.id ? saveLocal(entityName, saved) : localSaved; } catch (error) { console.warn(`[local-first] update ${entityName} local`, error); return localSaved; } },
    async bulkCreate(records = []) { const result = []; for (const record of records || []) result.push(await this.create(record)); return result; },
